...
import { Container, Search, Logo, Title, Carousel, Wrapper } from './homeStyles';
...
const Home = () => {
  ...
  const renderCarousel = () => {
    if (hasRestaurants) {
      return (
        <>
          <Title size="large">On your Region</Title>
          <Carousel {...settings}>
  ...
  return (
    <Wrapper>
      <Container>
        <Search>
          <Logo src={logo} alt="logo business" />
          <TextField
            outlined
            label="Search"
        ...
        <Modal open={open} onClose={() => setOpen(false)}>
          {restaurantSelected ? (
            <>
              ...
              <Text size="medium">
                {restaurantSelected?.opening_hours?.open_now ? 'Open :)' : 'Close :('}
              </Text>
            </>
          ) : (
            <>
